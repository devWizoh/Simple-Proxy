...
public class OnlineCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!(sender instanceof ProxiedPlayer)) {
            sender.sendMessage("");
            sender.sendMessage("§eJogadores em toda rede: §f" + all);
            sender.sendMessage("");
            return;
        }
        int list = 0;
        ...
        for (final String servers : Main.getInstance().getProxy().getServers().keySet()) {
            ...
            sender.sendMessage("§r §7■ §f" + serverinfo.getName().replaceAll("lobby", "Lobby #1").replaceAll("rankupteste", "R. Teste").replaceAll("login", "Login") + ": §7" + serverinfo.getPlayers().size());
            ++list;
        }
        if (list > 0) {
            sender.sendMessage("");
